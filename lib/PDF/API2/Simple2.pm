...
sub text {
  ...
  for (my $i = 0; $i < scalar(@words); $i++) {
    ...
    if ( $align eq 'center' ) {
      ...
      $words++ if $align eq 'both';
    }

    if ( $flush ) {
      if ('both' eq $align) {
        if (3<$words) {
          my $ws = $text_obj->wordspace();
          $text_obj->wordspace(($self->effective_width + $self->margin_left - $x)/($words -2));
          $self->_render_text_at( $text_obj, $sentance, $org_x, $y, 'left' );
          $text_obj->wordspace($ws);
        }
        else {
          $self->_render_text_at( $text_obj, $sentance, $org_x, $y, 'left' );
        }
